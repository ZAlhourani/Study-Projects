...
public class Exercise03_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds) {
        ...
        if (weightPounds <= 40) {
            return orderPrice = UP_TO_40_LB_RATE * weightPounds;
        }
        if (weightPounds > 40) {
            return orderPrice = OVER_40_LB_RATE * weightPounds;
        }
        return orderPrice;
    ...
    public double calculateShippingTotal(int weightPounds, boolean hasDiscount) {
        ...
        if (weightPounds <= 40) {
            orderPrice = UP_TO_40_LB_RATE * weightPounds;
        }
        if (weightPounds > 40) {
            orderPrice = OVER_40_LB_RATE * weightPounds;
        }
        if (hasDiscount) {
            discountAmount = orderPrice * discountRate;
