...
select * from park;   -- * means all of the column 
...
select city_name 
from city 
where state_abbreviation = 'CA';  -- filtering the data also we can break it to lines 
...
select distinct city_name from city; -- get rid of the duplicate 
...
select city_name from city where city_name like 'a%' -- ilike is case insesitive
...
-- this is example of between when we using dates 
