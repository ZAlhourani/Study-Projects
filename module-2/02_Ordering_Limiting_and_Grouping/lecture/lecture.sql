...
from state
order by census_region desc, state asc;
...
from city
order by city_name
offset 30 rows fetch next 10 rows only;  --30 is the starting row number
...
-- Show min and max park areas in same column (mulitple rows)
select park_name
from park
where area in (
               select min (area)
